...
let ActiveWorkflowManager = class ActiveWorkflowManager {
    constructor(logger, errorReporter, activeWorkflows, activeExecutions, externalHooks, nodeTypes, webhookService, workflowRepository, activationErrorsService, executionService, workflowStaticDataService, activeWorkflowsService, workflowExecutionService, instanceSettings, publisher, workflowsConfig, push) {
        this.logger = logger;
    ...
    async addWebhooks(workflow, additionalData, mode, activation) {
        ...
        for (const webhookData of webhooks) {
            ...
            try {
                ...
                if (['init', 'leadershipChange'].includes(activation) &&
    ...
    async clearWebhooks(workflowId) {
        ...
        if (workflowData === null) {
            throw new n8n_workflow_1.UnexpectedError('Could not find workflow', { extra: { workflowId } });
        }
    ...
    async activateWorkflow(workflowId, activationMode) {
        ...
        try {
            ...
            this.logger.error(`Issue on initial workflow activation try of ${(0, workflow_formatter_1.formatWorkflow)(dbWorkflow)} (startup)`, {
                error,
                workflowName: dbWorkflow.name,
                workflowId: dbWorkflow.id,
            });
    ...
    async add(workflowId, activationMode, existingWorkflow, { shouldPublish } = { shouldPublish: true }) {
        ...
        try {
            ...
            const canBeActivated = this.checkIfWorkflowCanBeActivated(workflow, constants_1.STARTING_NODES);
            if (!canBeActivated) {
                throw new n8n_workflow_1.WorkflowActivationError(`Workflow ${(0, workflow_formatter_1.formatWorkflow)(dbWorkflow)} has no node to start the workflow - at least one trigger, poller or webhook node is required`, { level: 'warning' });
            }
    ...
    async addTriggersAndPollers(dbWorkflow, workflow, { activationMode, executionMode, additionalData, }) {
        ...
        this.logger.debug(`Added triggers and pollers for workflow ${(0, workflow_formatter_1.formatWorkflow)(dbWorkflow)}`);
        return true;
    }
