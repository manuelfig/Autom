...
let WorkflowExecutionService = class WorkflowExecutionService {
    ...
    async executeManually({ workflowData, runData, startNodes, destinationNode, dirtyNodeNames, triggerToStartFrom, agentRequest, }, user, pushRef, streamingEnabled, httpResponse) {
        const pinData = workflowData.pinData;
        let pinnedTrigger = this.selectPinnedActivatorStarter(workflowData, startNodes?.map((nodeData) => nodeData.name), pinData, destinationNode);
        if (destinationNode) {
    ...
    async executeChatWorkflow(workflowData, executionData, user, httpResponse, streamingEnabled) {
        const data = {
            executionMode: 'chat',
            workflowData,
            userId: user.id,
