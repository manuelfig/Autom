    ...
    async getAuthUri(req) {
        ...
        if (oauthCredentials.useDynamicClientRegistration && oauthCredentials.serverUrl) {
            ...
            if (!authorization_endpoint || !token_endpoint || !registration_endpoint) {
                throw new bad_request_error_1.BadRequestError('The OAuth2 server does not support dynamic client registration. Missing endpoints in metadata.');
            }
            ...
            const { grantType, authentication } = this.selectGrantTypeAndAuthenticationMethod(data.grant_types_supported, data.token_endpoint_auth_methods_supported, data.code_challenge_methods_supported);
