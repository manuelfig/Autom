...
module.exports = {
    ...
    getWorkflow: [
        ...
        async (req, res) => {
            ...
            const workflow = await di_1.Container.get(workflow_finder_service_1.WorkflowFinderService).findWorkflowForUser(id, req.user, ['workflow:read'], { includeTags: !di_1.Container.get(config_1.GlobalConfig).tags.disabled });
            if (!workflow) {
                return res.status(404).json({ message: 'Not Found' });
            }
    ...
    updateWorkflow: [
        ...
        async (req, res) => {
            ...
            const workflow = await di_1.Container.get(workflow_finder_service_1.WorkflowFinderService).findWorkflowForUser(id, req.user, ['workflow:update']);
            if (!workflow) {
                return res.status(404).json({ message: 'Not Found' });
            }
            ...
            try {
                await (0, workflows_service_1.updateWorkflow)(workflow, updateData);
            }
            catch (error) {
    ...
    activateWorkflow: [
        ...
        async (req, res) => {
            ...
            if (!workflow.active) {
                ...
                di_1.Container.get(event_service_1.EventService).emit('workflow-activated', {
                    user: req.user,
    ...
    deactivateWorkflow: [
        ...
        async (req, res) => {
            ...
            if (workflow.active) {
                ...
                workflow.active = false;
                di_1.Container.get(event_service_1.EventService).emit('workflow-deactivated', {
                    user: req.user,
                    workflowId: workflow.id,
                    workflow,
                    publicApi: true,
