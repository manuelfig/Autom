...
let LicenseService = class LicenseService {
    ...
    async registerCommunityEdition({ userId, email, instanceId, instanceUrl, licenseType, }) {
        try {
            const { data: { licenseKey, ...rest }, } = await axios_1.default.post('https://enterprise.n8n.io/community-registered', {
                email,
                instanceId,
                instanceUrl,
                licenseType,
            ...
            if (e instanceof axios_1.AxiosError) {
                ...
                throw new bad_request_error_1.BadRequestError('Failed to register community edition: ' + errorMsg);
            }
            else {
                this.logger.error('Failed to register community edition', { error: (0, n8n_workflow_1.ensureError)(e) });
