...
let Server = class Server extends abstract_server_1.AbstractServer {
    ...
    async start() {
        if (!this.globalConfig.endpoints.disableUi) {
            ...
            await Promise.resolve().then(() => __importStar(require('./controllers/third-party-licenses.controller')));
        }
    ...
    async registerAdditionalControllers() {
        ...
        if (this.globalConfig.diagnostics.enabled) {
            await Promise.resolve().then(() => __importStar(require('./controllers/telemetry.controller')));
            await Promise.resolve().then(() => __importStar(require('./controllers/posthog.controller')));
        }
        try {
    ...
    async configure() {
        ...
        if (this.endpointPresetCredentials !== '') {
            const overwriteEndpointMiddleware = di_1.Container.get(credentials_overwrites_1.CredentialsOverwrites).getOverwriteEndpointMiddleware();
            ...
            this.app.post(`/${this.endpointPresetCredentials}`, async (req, res) => {
                if (!this.presetCredentialsLoaded || authenticationEnforced) {
                    ...
                    await di_1.Container.get(credentials_overwrites_1.CredentialsOverwrites).setData(body, true, true);
        ...
        protectedTypeFiles.forEach((path) => {
            this.app.get(path, authService.createAuthMiddleware({ allowSkipMFA: true, allowSkipPreviewAuth: true }), async (_, res) => {
        ...
        if (frontendService) {
            ...
            const securityHeadersMiddleware = (0, helmet_1.default)({
                ...
                xFrameOptions: isPreviewMode || constants_2.inE2ETests || backend_common_1.inDevelopment ? false : { action: 'sameorigin' },
        ...
        (0, n8n_core_1.installGlobalProxyAgent)();
    }
    configureSettingsRoute() {
        const { frontendService } = this;
        const authService = di_1.Container.get(auth_service_1.AuthService);
        if (frontendService) {
            this.app.get(`/${this.restEndpoint}/settings`, authService.createAuthMiddleware({ allowSkipMFA: false, allowUnauthenticated: true }), ResponseHelper.send(async (req) => {
                return req.user ? frontendService.getSettings() : frontendService.getPublicSettings();
            }));
        }
    }
    async initializeWorkflowIndexing() {
        if (di_1.Container.get(config_1.WorkflowsConfig).indexingEnabled) {
            if (di_1.Container.get(config_1.DatabaseConfig).isLegacySqlite) {
                this.logger.warn('Workflow indexing is disabled because legacy Sqlite databases are not supported. Please migrate the database to enable workflow indexing.');
                return;
            }
            const { WorkflowIndexService } = await Promise.resolve().then(() => __importStar(require('./modules/workflow-index/workflow-index.service')));
            di_1.Container.get(WorkflowIndexService).init();
        }
    }
    setupPushServer() {
