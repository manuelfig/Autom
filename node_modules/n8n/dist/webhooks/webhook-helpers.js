...
async function executeWebhook(workflow, webhookData, workflowData, workflowStartNode, executionMode, pushRef, runExecutionData, executionId, req, res, responseCallback, destinationNode) {
    ...
    const { responseMode, responseCode, responseData, checkAllMainOutputs } = evaluateResponseOptions(workflowStartNode, workflow, req, webhookData, executionMode, additionalKeys);
    ...
    try {
        ...
        if (responseMode === 'onReceived' && !didSendResponse) {
            const responseBody = (0, webhook_on_received_response_extractor_1.extractWebhookOnReceivedResponse)(responseData, webhookResultData);
        ...
        if (!didSendResponse) {
            executePromise
                ...
                const result = await (0, webhook_last_node_response_extractor_1.extractWebhookLastNodeResponse)(context, responseData, lastNodeTaskData, checkAllMainOutputs);
                if (!result.ok) {
...
function evaluateResponseOptions(workflowStartNode, workflow, req, webhookData, executionMode, additionalKeys) {
