...
const respondNodesResponseMode = {
  ...
  value: "responseNodes",
  description: "Send responses to the chat by using 'Respond to Chat' or 'Respond to Webhook' nodes"
};
...
class ChatTrigger extends import_n8n_workflow.Node {
  constructor() {
    super(...arguments);
    this.description = {
  ...
  async webhook(ctx) {
    ...
    try {
      await (0, import_GenericFunctions.validateAuth)(ctx);
    } catch (error) {
      if (error) {
    ...
    if (bodyData.action === "loadPreviousSession") {
      if (options?.loadPreviousSession === "memory") {
        const memory = await ctx.getInputConnectionData(import_n8n_workflow.NodeConnectionTypes.AiMemory, 0);
        const messages = (await memory?.chatHistory.getMessages() ?? []).filter((message) => !message?.additional_kwargs?.hideFromUI).map((message) => message?.toJSON());
        return {
