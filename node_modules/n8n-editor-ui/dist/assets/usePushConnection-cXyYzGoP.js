...
import { $ as getExecutionErrorToastConfiguration, At as isPrebuiltAgentTemplateId, Bo as useSettingsStore, Ht as useCredentialsStore, Ni as useUIStore, Q as getExecutionErrorMessage, Qr as getTriggerNodeServiceName, Qs as useRootStore, Sn as injectWorkflowState, Tn as useDocumentTitle, ci as parse, gr as isValidNodeConnectionType, gs as sendGetWorkerStatus, ji as codeNodeEditorEventBus, jt as isTutorialTemplateId, ki as globalLinkActionsEventBus, mn as useWorkflowHelpers, o as useWorkflowsStore, q as clearPopupWindowState, rt as useNodeTypesStore, t as useTelemetry, wt as SampleTemplates, xc as defineStore, xn as useNodeHelpers } from "./useTelemetry-DedEakwK.js";
import { t as useToast } from "./useToast-LQk5sJH0.js";
import { Sa as TRIMMED_TASK_DATA_CONNECTIONS_KEY, Ui as isCommunityPackageName, fa as EVALUATION_TRIGGER_NODE_TYPE, pi as generateNodesGraph, qo as WORKFLOW_SETTINGS_MODAL_KEY } from "./constants-C5Tj3dXQ.js";
import { i as useBuilderStore, t as useAssistantStore } from "./assistant.store-BtplYeLr.js";
...
import { i as usePushConnectionStore, t as useRunWorkflow } from "./useRunWorkflow-BLqckUNU.js";
...
import { t as useReadyToRunWorkflowsV2Store } from "./readyToRunWorkflowsV2.store-BJAu2HwR.js";
...
async function executionFinished({ data }, options) {
	...
	if (templateId) {
		...
		else if (templateId.startsWith("37_onboarding_experiments_batch_aug11")) readyToRunWorkflowsStore.trackExecuteWorkflow(templateId.split("-").pop() ?? "", data.status);
		else if (templateId === "ready-to-run-ai-workflow-v1" || templateId === "ready-to-run-ai-workflow-v2" || templateId === "ready-to-run-ai-workflow-v3" || templateId === "ready-to-run-ai-workflow-v4") if (data.status === "success") readyToRunWorkflowsV2Store.trackExecuteAiWorkflowSuccess();
		else readyToRunWorkflowsV2Store.trackExecuteAiWorkflow(data.status);
